...
function extractListingId(offerId) {
  try {
    ...
    if (parts[0] !== '3' || parts.length < 2) return null;
...
export function classifyResaleListings(facets, options = {}) {
  ...
  for (const facet of facets) {
    ...
    for (const offerId of (facet.offers || [])) {
      ...
      if (listingId !== null) offerListingMap.set(offerId, listingId);
    }
  }

  if (offerListingMap.size === 0) return new Map();
  ...
  for (const cluster of clusters) {
    if (cluster.length >= minClusterSize) {
      for (const entry of cluster) brokerOfferIds.add(entry.offerId);
    }
  }
