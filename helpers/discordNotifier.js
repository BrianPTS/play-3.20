...
export async function sendInventoryAlert({ eventName, venue, eventId, newStandardSeats, priceDrops }) {
  ...
  if (hasNew) {
    ...
    const lines = newStandardSeats.slice(0, 15).map(s =>
      `**${s.section}** Row ${s.row} · ${s.quantity} seat${s.quantity > 1 ? 's' : ''} · $${s.price.toFixed(2)}`
    );
    ...
    embeds.push({
      ...
      color: 0x22c55e,
      footer: { text: `${eventName} · ${venue || 'Unknown venue'}` },
  ...
  if (hasDrops) {
    const lines = priceDrops.slice(0, 15).map(d => {
      ...
      return `${tag} **${d.section}** Row ${d.row} · ${d.quantity} seat${d.quantity > 1 ? 's' : ''} · ${arrow} (−${pct}%)`;
    });
